...
import { Search, RefreshCw, Building2, AlertCircle, Loader2, ShieldCheck, ChevronRight, Package } from "lucide-react"
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary">{total} registered seller{total !== 1 ? "s" : ""}</p>
        </div>
        ...
        {KYC_TABS.map((tab) => (
          <button
            ...
            className={cn(
              ...
              kycFilter === tab.value ? "border-ocean text-ocean" : "border-transparent text-text-secondary hover:text-text-primary"
            )}
          >
            {tab.label}
          </button>
      ...
      {isLoading && items.length === 0 ? (
        <div className="flex items-center justify-center py-16 gap-2 text-text-secondary text-sm">
          <Loader2 className="w-5 h-5 animate-spin" /> Loading sellers…
        </div>
      ) : items.length === 0 ? (
        <div className="bg-white rounded-xl border border-border p-12 text-center">
          <Building2 className="w-10 h-10 text-gray-200 mx-auto mb-3" />
          <p className="text-text-secondary">No sellers found</p>
        </div>
      ) : (
        <div className="space-y-2">
          {items.map((seller) => {
            const kyc = KYC_CFG[seller.kyc_status ?? "not_started"] ?? KYC_CFG.not_started
            const totalListings = (seller as any).total_listings ?? 0
            const activeListings = (seller as any).active_listings ?? 0
            return (
                ...
                <div className="bg-white rounded-xl border border-border shadow-sm px-5 py-4 flex items-center gap-4 hover:border-ocean/30 hover:shadow-md transition-all cursor-pointer group">
                  <div className="w-10 h-10 rounded-full bg-navy/10 flex items-center justify-center shrink-0">
                    <span className="text-navy font-bold text-sm">
                      {(seller.company_name || seller.full_name || seller.email || "?")[0].toUpperCase()}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-text-primary">{seller.company_name || seller.full_name || "—"}</p>
                      {!seller.is_active && <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full border border-gray-200">Inactive</span>}
                    </div>
                    <p className="text-xs text-text-secondary truncate">{seller.email} {seller.country ? `· ${seller.country}` : ""}</p>
                  </div>
                  <div className="hidden sm:flex items-center gap-6 text-xs text-text-secondary shrink-0">
                    <div className="text-center">
                      <p className="font-semibold text-text-primary">{totalListings}</p>
                      <p>Listings</p>
                    </div>
                    <div className="text-center">
                      <p className="font-semibold text-success">{activeListings}</p>
                      <p>Active</p>
                    </div>
                    <div className="text-center">
                      <p className="font-semibold text-text-primary">{(seller as any).total_deals ?? 0}</p>
                      <p>Deals</p>
                    </div>
                    <div className="text-center">
                      <p className="font-semibold text-text-primary">{fmtDate(seller.created_at)}</p>
                      <p>Joined</p>
                    </div>
                  </div>
