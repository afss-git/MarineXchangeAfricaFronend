...
export function Hero() {
  ...
  // Load top-level categories for chips
  useEffect(() => {
      ...
      try {
        ...
        const catSuggs: Suggestion[] = categories
          ...
          .map(c => ({
            ...
            label: c.name,
            sub: "Browse category",
            href: `/listings?category_id=${c.id}`,
          }))
  ...
  return (
    <section
      className="relative min-h-[90vh] pt-16 overflow-hidden flex items-center"
    ...
    >
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at center, rgba(14,165,233,0.09) 0%, transparent 70%)",
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 w-full">
        {/* Badge */}
        <div
          className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full mb-7 ${visible ? "animate-fade-up" : "opacity-0"}`}
        ...
        {/* Headline */}
        <h1
          className={`text-white font-extrabold leading-[1.08] mb-5 max-w-180 ${visible ? "animate-fade-up delay-1" : "opacity-0"}`}
        ...
        {/* Subheading */}
        <p
          className={`text-white/55 text-base sm:text-lg max-w-130 mb-10 leading-relaxed ${visible ? "animate-fade-up delay-2" : "opacity-0"}`}
        >
        ...
        {/* ── Search bar ────────────────────────────────────────────────── */}
        <div
          className={`relative max-w-[680px] mb-7 ${visible ? "animate-fade-up delay-3" : "opacity-0"}`}
        >
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <input
                ...
                placeholder="Search vessels, offshore equipment, machinery…"
                className="flex-1 bg-transparent text-white placeholder-white/35 text-sm sm:text-base px-3 py-4 focus:outline-none"
                autoComplete="off"
              />
              {query && (
                <button
                  ...
                  className="mr-2 p-1.5 rounded-lg text-white/40 hover:text-white/70 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
              {loadingSugg && <Loader2 className="w-4 h-4 text-white/40 animate-spin mr-3" />}
              <button
                type="submit"
                className="m-1.5 px-5 py-2.5 rounded-xl bg-ocean hover:bg-ocean-dark text-white text-sm font-semibold transition-colors shrink-0"
              >
                Search
              </button>
            </div>
          ...
          {showDropdown && suggestions.length > 0 && (
            ...
            >
              {suggestions.map((s, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {s.sub && <p className="text-text-secondary text-xs">{s.sub}</p>}
                  </div>
              ...
              <Link
                href={`/listings?q=${encodeURIComponent(query.trim())}`}
                onClick={() => { setShowDropdown(false); setQuery("") }}
        ...
        {/* ── Category chips ─────────────────────────────────────────────── */}
        {categories.length > 0 && (
          <div className={`flex flex-wrap gap-2 mb-12 ${visible ? "animate-fade-up delay-4" : "opacity-0"}`}>
            <span className="text-white/35 text-xs font-medium self-center mr-1">Browse:</span>
            {categories.map(cat => (
              <Link
                key={cat.id}
                href={`/listings?category_id=${cat.id}`}
                className="px-3.5 py-1.5 rounded-full text-xs font-semibold text-white/70 hover:text-white transition-all"
                style={{
                  background: "rgba(255,255,255,0.06)",
            ...
            <Link
              href="/listings"
              className="px-3.5 py-1.5 rounded-full text-xs font-semibold text-ocean transition-all"
        ...
        {/* ── CTA buttons ────────────────────────────────────────────────── */}
        <div className={`flex flex-wrap gap-3 ${visible ? "animate-fade-up delay-5" : "opacity-0"}`}>
          <Link href="/signup/buyer">
            <Button
              size="lg"
              className="bg-ocean hover:bg-ocean-dark text-white px-7 py-5 text-sm font-semibold transition-all hover:-translate-y-0.5"
            >
              Start as Buyer
          ...
          <Link href="/signup/seller">
            <Button
              size="lg"
              variant="outline"
              className="border-white/25 text-white bg-transparent hover:bg-white/8 px-7 py-5 text-sm font-semibold transition-all hover:-translate-y-0.5"
            >
              List Your Assets
            </Button>
          </Link>
